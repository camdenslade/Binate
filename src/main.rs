...
use binary::{BinaryProvider, MmapBinaryProvider};
use clap::{Parser, Subcommand, ValueEnum};
use diff::{DiffConfig, SemanticDiff};
use normalize::NormalizerChain;
    ...
    match cli.command {
        Commands::Compare {
            ...
            let normalizer = if no_normalize {
                ...
                let defaults: Vec<Box<dyn normalize::Normalizer>> = vec![
                    Box::new(normalize::BuildIdNormalizer),
                    Box::new(normalize::TimestampNormalizer),
                    Box::new(normalize::AbsolutePathNormalizer::new()),
                    Box::new(normalize::LinkerVersionNormalizer),
                ];
