...
// Compile-time check: DiffResult must be Send + Sync so it can cross thread boundaries.
const _: fn() = || {
...
impl SemanticDiff {
    /// Compare two binaries by file path using default normalization.
    /// Convenience wrapper around [`compare`] for the common case.
